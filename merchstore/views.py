...
class ProductDetailView(DetailView):
    ...
    model = Product
    success_url = reverse_lazy("merchstore:cart_list")
        ...
        if not request.user.is_authenticated:
            ...
            if len(request.POST.get("amount")) != 0 and affected_product.stock >= int(
                ...
                if (
                    ...
                    and request.user.profile != affected_product.owner
                ):
                    transaction.save()
                    affected_product.reduce_stock(transaction.amount)
                else:
                    return redirect(reverse("merchstore:product_list"))
            else:
...
class ProductUpdateView(LoginRequiredMixin, UpdateView):
    ...
    def get(self, request, *args, **kwargs):
        affected_product = get_object_or_404(Product, pk=self.kwargs["pk"])
        if request.user.profile != affected_product.owner:
            return redirect(reverse_lazy("merchstore:product_list"))
